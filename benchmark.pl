...
% Report = report(Games, Moves, WX, WO, D, TimeSec, Inferences)
benchmark_games(TypeX, TypeO, N, AltStart, MaxMoves, Report) :-
    ...
    bench_loop(1, N, TypeX, TypeO, AltStart, MaxMoves,
               ...
               WX, WO, D, Moves, _Aborted),
    get_time(T1),
    statistics(inferences, I1),
    Time is T1 - T0,
    Inf is I1 - I0,
    Report = report(N, Moves, WX, WO, D, Time, Inf).

print_report(report(G, M, WX, WO, D, Time, Inf)) :-
    format("=== Benchmark ===~n", []),
    format("Games      : ~w~n", [G]),
...
bench_loop(I, N, TX, TO, Alt, MaxMoves,
    ...
    bench_game(Board0, Start, TX, TO, 0, MaxMoves, Result, MovesInGame),
...
bench_game(Board, Player, TX, TO, Moves0, MaxMoves, Result, MovesOut) :-
    ...
    choose_move(Type, Player, Board, Col),
